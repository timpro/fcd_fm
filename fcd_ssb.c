...
void downsample(int16_t* buf, int16_t *I, int16_t *Q, int len)
{
	...
	for (i = pos = 0; pos < len - 7; pos += 8) {
		I[i] = (buf[pos+0] - buf[pos+3] + buf[pos+7] - buf[pos+4]);
		Q[i] = (buf[pos+1] - buf[pos+5] + buf[pos+2] - buf[pos+6]);
		i++;
	}
}

void demodulate(int16_t audio[], int16_t I[], int16_t Q[], int len)
{
	// taken from rtl_fm: (I + Q) is not SSB, and gives bad aliasing
	for (int i = 0; i < len; i++)
		audio[i] = (I[i] + Q[i]);
}
...
int work()
{
	...
	void *out;
	int16_t i48khz[BLOCKSIZE];
	int16_t q48khz[BLOCKSIZE];
